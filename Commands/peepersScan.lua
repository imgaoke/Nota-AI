...
function Run(self, units, parameter)	
	...
	if not self.formationInitialized then
		local interval = Game.mapSizeX / (#parameter.peepers - 1)
		for i = 1, #parameter.peepers do
			Spring.GiveOrderToUnit(parameter.peepers[i], CMD.MOVE,{interval * (i - 1), 0, 0},{"shift"})
		end
		self.formationInitialized = true
	elseif readyToGiveScanCommand == true then
		local interval = Game.mapSizeX / (#parameter.peepers - 1)
		for i = 1, #parameter.peepers do
			Spring.GiveOrderToUnit(parameter.peepers[i], CMD.MOVE,{interval * (i - 1), 0, Game.mapSizeZ},{"shift"})
		end
		readyToGiveScanCommand = false
		...
		for i = 1, #parameter.peepers do
			local velX, velY, velZ, velLength = Spring.GetUnitVelocity(parameter.peepers[i])
			if Spring.ValidUnitID(parameter.peepers[i]) and velLength ~= 0 then
				allStopped = false
				break
			end
		end
		...
		if allStopped == true and scanCommandGiven == false then
			readyToGiveScanCommand = true
		elseif allArrived == true and scanCommandGiven == true then
			return SUCCESS
		end
