...
function Run(self, units, parameter)	
	...
	local x, y, z = Spring.GetUnitPosition(parameter.farckID)
	-- first time
	if not self.initialized then
		...
		Spring.GiveOrderToUnit(parameter.farckID, CMD.MOVE, {self.nextSpot[1], y, self.nextSpot[2]},{"shift"})
	end
	...
	if math.abs(x - self.nextSpot[1]) <= 500 and math.abs(z - self.nextSpot[2]) <= 500 then
