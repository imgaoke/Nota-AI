...
function BFSPathFinding(dangerGridMapLayered, missionInformation)
			...
			for _, value in ipairs(toBeVisited) do
				...
				for i = -1, 1 do
					for k = -1, 1 do
						...
						parent[value[1] + i][value[2] + k] == nil then
							allNeighboursAreOpened = false
							--[[
							if value[1] == 97 and value[2] == 58 then
								Spring.Echo("97, 58 added here")
							end
							if value[1] == 96 and value[2] == 58 then
								Spring.Echo("96, 58 added here")
							end
							]]--

							--Spring.Echo(value[1] .. "," .. value[2] .. "added here")
							
							--if toBeVisitedNextMark[value[1]][value[2]] == nil then
							--	table.insert(toBeVisitedNext, {value[1], value[2]})
							--	toBeVisitedNextMark[value[1]][value[2]] = true
							--end
							table.insert(toBeVisitedNext, {value[1], value[2]})
				...
				if allNeighboursAreOpened == true then
...
return function(dangerGridMapLayered, missionInformation)
	--local missionInformation = core.MissionInfo()
	local parent = BFSPathFinding(dangerGridMapLayered, missionInformation)
	--[[
	local xGrid = math.ceil(Game.mapSizeX / 50)
	local zGrid = math.ceil(Game.mapSizeZ / 50)
	local count = 0
