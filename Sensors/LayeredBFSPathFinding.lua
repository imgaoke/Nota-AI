...
function BFSPathFinding(dangerGridMap, missionInformation)
	...
	while true do
		...
		local thereIsGridForCurrentDangerLevel = false
		local currentDangerLevel = 1
		while thereIsGridForCurrentDangerLevel == false and currentDangerLevel <= 5 do
			for _, value in ipairs(toBeVisited) do
				...
				if visited[value[1]][value[2]] == nil then
						...
						for k = -1, 1 do
							if (not (i == 0 and k == 0)) and value[1] + i >= 1 and value[1] + i <= xGrid and value[2] + k >= 1 and value[2] + k <= zGrid then
								...
								if (visited[value[1] + i][value[2] + k] == nil) then
									...
									if dangerGridMapLayered[value[1] + i][value[2] + k] <= currentDangerLevel then
										--Spring.Echo("here2")
										if toBeVisitedMark[value[1] + i][value[2] + k] == nil then
											--Spring.Echo("here3")
											...
											toBeVisitedMark[value[1] + i][value[2] + k] = true
											thereIsGridForCurrentDangerLevel = true
					...
					local allNeighboursAreParented = true
					for i = -1, 1 do
						for k = -1, 1 do
							if  (not (i == 0 and k == 0)) and value[1] + i >= 1 and value[1] + i <= xGrid and value[2] + k >= 1 and value[2] + k <= zGrid and parent[value[1] + i][value[2] + k] == nil then
								allNeighboursAreParented = false
								break
							end
						end
					...
					if allNeighboursAreParented then
